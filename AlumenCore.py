...
class SmartTimeRemainingColumn(ProgressColumn):
    ...
    def __init__(self, is_cached_list, rpm_limit=None):
        ...
        self.avg_api_time = 2.0
        if rpm_limit and rpm_limit > 0:
            self.avg_api_time = max(2.0, 60.0 / rpm_limit)
...
def initialize_api_keys_and_model():
    ...
    if env_keys:
        keys_from_env = [k.strip() for k in env_keys.split(',') if k.strip()]
        if keys_from_env:
            available_api_keys.extend(keys_from_env)
            console.print(f"✅ Trovate {len(keys_from_env)} API key dalla variabile d'ambiente 'GEMINI_API_KEY'.")
...
def set_rpm_limit_func(rpm_str, is_telegram: bool = False):
    if script_args.ollama_model:
        msg = "ℹ️  La gestione RPM è disabilitata in modalità Ollama."
        if is_telegram: return msg
        console.print(f"[yellow]{msg}[/]")
        return
...
def set_model_name(model_name, is_telegram: bool = False):
    ...
    if script_args.ollama_model:
        msg = "ℹ️  Non puoi cambiare modello in modalità Ollama. Riavvia lo script con un diverso --ollama-model."
        if is_telegram: return msg
        console.print(f"[yellow]{msg}[/]")
        return
    if not model_name:
...
def show_stats(title="📊 STATISTICHE DI ESECUZIONE", is_telegram: bool = False):
    ...
    if is_telegram:
        ...
        lines.append(f"🔢 *Token Input:* {total_input_tokens}")
        lines.append(f"🔠 *Token Output:* {total_output_tokens}")
        lines.append(f"💰 *Token Totali:* {total_input_tokens + total_output_tokens}")
        lines.append("")
        lines.append(f"💾 *Traduzioni da cache:* {cache_hit_count}")
        if script_args.ollama_model:
            lines.append(f"🚀 *Chiamate al modello locale:* {total_api_calls}")
            lines.append(f"🤖 *Modello Ollama:* `{script_args.ollama_model}`")
        else:
        ...
        if script_args.ollama_model:
            title = "📊 STATISTICHE DI ESECUZIONE (OLLAMA)"
        else:
            title = "📊 STATISTICHE DI ESECUZIONE (GEMINI)"
        ...
        main_table.add_row("🔢 Token Input", str(total_input_tokens))
        main_table.add_row("🔠 Token Output", str(total_output_tokens))
        main_table.add_row("💰 Token Totali", str(total_input_tokens + total_output_tokens))
        main_table.add_section()
        main_table.add_row("💾 Voci da cache", str(cache_hit_count))
        if script_args.ollama_model:
            main_table.add_row("🚀 Chiamate al modello locale", str(total_api_calls))
            main_table.add_row("🤖 Modello Ollama", script_args.ollama_model)
        else:
...
def wait_for_rpm_limit():
    global rpm_limit, rpm_request_timestamps
    if not rpm_limit or rpm_limit <= 0 or script_args.ollama_model: return
    while True:
...
def _perform_genai_api_call(prompt):
    ...
    if hasattr(response_obj, 'usage_metadata'):
        ...
        total_output_tokens += response_obj.usage_metadata.candidates_token_count
...
def _call_ollama_model(prompt_text):
    ...
    api_url = f"{script_args.ollama_url}/api/generate"
    payload = {
        "model": script_args.ollama_model,
        "prompt": prompt_text,
...
if __name__ == "__main__":
    ...
    if not args_parsed_main.ollama_model and not check_internet_connection():
        console.print("[bold red]🛑 ERRORE: Nessuna connessione internet rilevata.[/]")
        console.print("Lo script richiede una connessione attiva per comunicare con le API di Gemini.")
        sys.exit(1)

    if args_parsed_main.ollama_model and args_parsed_main.api:
        log_critical_error_and_exit("Non puoi specificare sia --ollama-model che --api. Scegli una delle due modalità.")
        ...
        if interrupted:
            ...
            if args_parsed_main.shutdown:
                ...
                try:
                    ...
                    console.print(f"[red]Errore nell'esecuzione del comando di spegnimento: {e}[/]")
